...
class StockCheckerServiceClient(Node):
    ...
    def send_stock_check_request(self, item_name):
        ...
        future.add_done_callback(lambda future: self.get_stock_level_callback(future, item_name))

    def get_stock_level_callback(self, future, item_name):
        try:
            response = future.result()
            self.stock_levels[item_name] = response.stock_level
